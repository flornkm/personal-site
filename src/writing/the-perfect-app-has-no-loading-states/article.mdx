...
One thing becomes pretty clear: on the web, a first visit will always have a loading state when content is dynamic. Whether it happens upfront (the data is fetched on the server and the first paint already contains it, which many people do on public facing sites for SEO) or on the client, there will be some kind of delay. Server-side caching makes this pretty fast, but the initial document is still larger than a shell that shows a root-level spinner right away.
